...
class BottleneckBlock:
    ...
    def forward(self, x: np.ndarray) -> np.ndarray:
        ...
        """
        # YOUR CODE HERE
        # h1 = relu(np.einsum('nchw,ck->nkhw', x, self.W1))
        # h2 = relu(np.einsum('nchw,ck->nkhw', h1, self.W2))
        # h3 = np.einsum('nchw,ck->nkhw', h2, self.W3)
        # s =  np.einsum('nchw,ck->nkhw', x, self.Ws) if self.Ws else x
        # y = relu(h3 + s)
        
        h1 = relu(x @ self.W1)
        h2 = relu(h1 @ self.W2)
        h3 = h2 @ self.W3
        s =  relu(x @ self.Ws) if self.Ws is not None else x
        y = relu(h3 + s)
        return y
